...
try
{
  ...
  New-Item -Type Directory -Name "Series.tech-$(Get-Date -UFormat '%Y-%m-%d_%H-%M-%S')" |
  Convert-Path |
  Set-Location
}
catch
...
{
  if ((Read-Host -Prompt 'In order to install Series.tech, your Spotify client must be updated. Do you want to continue? (Y/N)') -ne 'y')
  {
    exit
  }
}
...
{
  Write-Host 'Downloader seneste version af Spotify...'
...
Write-Host "Downloader seneste patch (chrome_elf.zip)...`n"
